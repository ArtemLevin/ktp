...
const assessmentCard=content?.assessments?.enabled?`<section class="card" data-assessment-topic-link="true"><span class="kicker">Проверочные материалы</span><h2>${esc(content.assessments.title||'Проверочные материалы')}</h2><p>${esc(content.assessments.description||'')}</p><a class="cta" href="${esc(content.assessments.independentHref||'#')}">Самостоятельная работа →</a> <a class="cta" href="${esc(content.assessments.controlHref||'#')}">Контрольная работа →</a></section>`:'';
...
window.KTP_INJECT_TOPIC_ASSESSMENTS?.();
})();
