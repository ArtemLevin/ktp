...
document.title=`${lessons.length} уроков: ${topicTitle} — ${meta.grade} класс · KTP 3.0`;
app.innerHTML=`<main class="lesson-page catalog-page"><nav class="topnav"><a href="${esc(meta.topicHref||'#')}">← Страница темы</a><div><a href="${esc(meta.ktpHref||'#')}">КТП 3.0</a></div></nav><header class="lesson-hero catalog-hero"><div class="eyebrow">${esc(meta.grade)} класс · ${esc(meta.subject)} · ${esc(meta.book)}</div><h1><span>Поурочная подготовка</span>${esc(topicTitle)}</h1><div class="hero-pills"><span>${lessons.length} уроков</span><span>${weeks.length} учебных недель</span><span>Тема ${esc(meta.topicNumber)} из ${row?.topics?.length||'—'}</span><span>${esc(meta.schoolYear)}</span></div><p>Последовательный маршрут темы: каждый урок имеет собственную адресную страницу, теорию, тренировку, домашнюю работу и проверочные варианты.</p><a class="hero-cta" href="01.html">Начать с урока 1 →</a></header>${problems.length?`<section class="integrity-warning"><b>Проверка данных:</b> ${problems.map(esc).join(' ')}</section>`:''}<section class="catalog-summary"><article><b>${lessons.length}</b><span>поурочных модулей</span></article><article><b>${weeks.length}</b><span>учебных недель</span></article><article><b>${lessons.reduce((s,l)=>s+(l.independent?.variants?.length||0),0)}</b><span>вариантов самостоятельных</span></article><article><b>${lessons.reduce((s,l)=>s+(l.control?.variants?.length||0),0)}</b><span>вариантов контроля</span></article></section><div class="weeks">${groups}</div><nav class="lesson-prev-next"><a href="${esc(meta.topicHref||'#')}">← К теме KTP</a><span></span><a href="${esc(meta.ktpHref||'#')}">К календарю →</a></nav></main>`;
})();
