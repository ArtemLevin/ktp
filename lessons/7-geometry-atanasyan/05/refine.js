...
if(l53){
 patch(l53,t=>String(t.text||'').startsWith('Почему внешний вид чертежа сам по себе не доказывает касание?'),t=>({...t,answer:'Нужно установить, что у прямой и окружности ровно одна общая точка.'}));
 patch(l53,t=>String(t.text||'').includes('одну общую точку'),t=>({...t,text:String(t.text).replace('одну общую точку','ровно одну общую точку')}));
}
})();
