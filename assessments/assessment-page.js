...
if(!data||!topic||!work||!app){
  if(app) app.innerHTML='<main class="shell"><section class="panel"><h1>Материал не найден</h1><p>Проверьте подключение данных и параметры страницы.</p></section></main>';
  return;
}
...
function variantHtml(v){const skills=skillsOf(v);return `<section class="variant ${v.id===selected?'active':''}" data-variant="${v.id}"><header class="paper-head"><div><div class="paper-kicker">${esc(data.meta.grade)} класс · ${esc(data.meta.subject)} · ${esc(data.meta.book)}</div><h2>${esc(typeLabel)} · вариант ${v.id}</h2><p>${esc(topic.title)}</p></div><div class="student-fields"><span>ФИО ____________________________</span><span>Дата __________</span></div></header><div class="paper-meta"><span>${esc(work.duration)}</span><span>Максимум ${work.maxScore} баллов</span><span>Калькулятор не требуется</span></div><div class="instruction"><b>Инструкция.</b> Записывайте основные преобразования. В заданиях уровня C важен ход рассуждения: полный балл ставится за математически обоснованное решение.</div><div class="skill-strip">${skills.map(s=>`<span>${esc(s)}</span>`).join('')}</div><div class="tasks">${v.tasks.map(taskHtml).join('')}</div>${keyHtml(v)}</section>`;}
