...
const response=await page.goto('http://127.0.0.1:'+port+'/topics/10-geometry-atanasyan/05.html',{waitUntil:'networkidle',timeout:30000});
...
if(state.text<8000)throw new Error('topic rendered text too short '+state.text);
if(state.spatial<6)throw new Error('expected >=6 rendered spatial scenes, got '+state.spatial);
if(errors.length)throw new Error(errors.join(' | '));

const body=await page.locator('body').innerText();
for(const token of [
  'Универсальный маршрут решения',
  'Объём: сначала модель и высота',
  'Сечение: грань за гранью',
  'Карта параллельности и перпендикулярности',
  'Расстояние: ищем перпендикуляр',
  'Угол прямой с плоскостью',
  'Двугранный угол',
  'Подобные тела и размерность',
  'Диагностика ошибки'
...
console.log('Grade 10 Atanasyan topic 05 core browser QA passed: topic, spatial scenes, mobile and print.');
await browser.close();
