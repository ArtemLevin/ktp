...
assert(section.includes('полностью готово'),'Plan must retain topic05 as complete');
assert(section.includes('48–60'),'Plan topic05 lesson range missing');
for(const token of ['21–23','п. 47','68–69','п. 72','74–75'])assert(section.includes(token),`Plan topic05 source missing ${token}`);
const mapRow=map.split('\n').find(line=>line.startsWith('| 05 | Геометрические места точек. Симметрия |'))||'';
assert(mapRow.includes('04.03–22.04.2027')&&mapRow.includes('48–60')&&mapRow.includes('пп. 68–69, 72, 74–75')&&mapRow.includes('**full**'),'content map topic05 drift');
